...
Before installing a VM, we need to first download Debian Sarge ISO files for the IA-32 platform. You can download binary ISO files from [[https://archive.org/details/debian-sarge-r8-32bit][here]] and optionally download the source ISO files from [[https://archive.org/details/debian_3.1r8_source][here]]. The source ISO files are not required by this book.

*** VM Creation
After downloading the ISO files, open =virt-manager= and create a virtual machine following the steps below.
...
2. You can add extra DVD ISOs to =/etc/apt/sources.list= by running =sudo apt-cdrom add= loading extra ISO files.
3. Blacklist the builtin RTL8139 driver so that you can work on your own driver without worrying about conflicts.
...
4. Since we blacklisted the builtin Ethernet driver, the VM does not have a network connection. To share files between the host computer and the VM, we will use a virtual USB drive.
