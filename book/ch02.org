...
3. Since we blacklisted the builtin Ethernet driver, the VM does not have a network connection. To share files between the host computer and the VM, we will use a serial utility.
   1. Shut down the VM and click the lightbulb icon. Click "Add Hardware" and then click "Serial". In "Device Type", select "Pseudo TTY (pty)". A device named "Serial 2" is added.
   2. Start the VM. Make sure your account is in group =dialout= by running command =groups= to check. If not, run =sudo adduser zengxu dialout=, replacing =zengxu= with your account name.
   3. Install =lrzsz= by =sudo apt-get install lrzsz= on the VM and =sudo apt install lrzsz= on the host computer. Create scripts =send_to_host.sh=, =recv_from_host.sh= on the VM and =send_to_vm.sh=, =recv_from_vm.sh= on the host computer. To send a file from the host computer to the VM, run =./recv_from_host.sh= on the VM and then run =sudo ./send_to_vm.sh <file>=. To send a file from the VM to the host computer, run =sudo ./recv_from_vm.sh= on the host computer and =./send_to_host.sh <file>= on the VM.
